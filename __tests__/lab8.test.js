describe('Basic user flow for SPA ', () => {
  ...
  it('Test5: On first Entry page - checking <entry-page> contents', async () => {
    /*
      ...
      */
      ...
      const entry = await page.$eval("entry-page", (response) => {
        return response.entry;
      });
      expect(expected).toEqual(entry);
  }, 10000);

  it('Test6: On first Entry page - checking <body> element classes', async () => {
    // implement test6: Clicking on the first journal entry should update the class attribute of <body> to ‘single-entry’
    const elementClass = await page.$eval('body', (response) => {
      return response.className;
    });
    expect(elementClass).toBe('single-entry');
  });
  ...
  it('Test8: On Settings page - checking page header title', async () => {
    ...
    const title = await page.$eval("h1", (response) => { return response.innerHTML; });
  ...
  it('Test17:  click journal 3, new URL should contain /#entry3', async () => {
    page.goBack();
    const entries = await page.$$('journal-entry');
    entries[2].click();
    await page.waitForNavigation();
  ...
  it('Test18: check title for entry 3', async () => {
    const title = await page.$eval("h1", (response) => { return response.innerHTML; });
  ...
  const entry = await page.$eval("entry-page", (response) => {return response.entry; });
  expect(expected).toEqual(entry); 
  }, 10000);
